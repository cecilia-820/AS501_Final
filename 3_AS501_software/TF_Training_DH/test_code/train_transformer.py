...
class Seq2SeqTransformer(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        tm = nn.Transformer.generate_square_subsequent_mask(tgt.size(1), device=src.device)
...
def beam_search(model, src_sent, sv, tv, device, beam=4, max_len=MAX_TGT_LEN, alpha=0.6):
    ...
    for _ in range(max_len):
        ...
        for score, ids in beams:
            ...
            tm  = nn.Transformer.generate_square_subsequent_mask(len(ids), device=device)
...
def export_weights(model, out_dir, sv, tv):
    ...
    with open(path,'w') as f:
        ...
        for i,a in [(1,'norm1'),(2,'norm2')]:
            ...
            wca(f,f"enc_ln{i}_beta_q", ec(lambda l,a=a:q12(getattr(model.decoder.layers[l],a).bias)))
...
def export_input_data(sv, tv, src_sents, tgt_sents, out_dir, n=1):
    pairs = sorted(zip(src_sents,tgt_sents), key=lambda x:len(x[0].split()))[:n]
    sd, so = [], [0]
    for s,_ in pairs:
        ids = sv.encode(s,MAX_SRC_LEN)[1:-1]
