...
static I64Pair i64_mul_i32(int32_t a, int32_t b) {
    ...
    uint32_t lo = p0 + (p1 << 16), hi = p3 + (p1 >> 16) + (p2 >> 16);
    uint32_t old_lo = lo;
    lo += (p2 << 16);
    if (lo < old_lo) hi++;
    I64Pair r = {lo, hi};
...
static void layer_norm_q12(const int32_t *x,  int32_t *out,
    ...
    uint32_t std_q12  = isqrt32(uvar) + (uint32_t)TF_LN_EPS_Q;
...
#ifdef RISCV
////////////////////////////////////////////////////////////////////////////////
// GEMV Accelerator MMIO interface
//
// ACCEL_START_W = 0x0810_0000 → byte address = 0x2040_0000
// The accelerator computes: out[row] = (Σ_k mat[row*cols+k] * vec[k]) >> 12
// All addresses are absolute DMEM byte addresses.
////////////////////////////////////////////////////////////////////////////////
...
static void gemv_hw(const int32_t *mat, const int32_t *vec,
...
static void matmul_q12(const int32_t *A, const int32_t *Bt,
                        ...
                        int M, int K, int N) {
#ifdef RISCV
    /* Hardware-accelerated: for each row m of A, the accelerator computes
     * C[m,:] = Bt[N×K] × A[m,:] as a single GEMV (N dot products of length K).
     * The vector (A row) is loaded once, then streamed against all N weight rows. */
    for (int m = 0; m < M; ++m)
        gemv_hw(Bt, A + m * K, C + m * N, N, K);
#else
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "--max-sentences") == 0 && i + 1 < argc) {
            args.max_sentences = atoi(argv[++i]);
        } else if (strcmp(argv[i], "--out-hyp") == 0 && i + 1 < argc) {
            args.out_hyp = argv[++i];
        } else if (strcmp(argv[i], "--out-result-txt") == 0 && i + 1 < argc) {
            args.out_result_txt = argv[++i];
        } else if (strcmp(argv[i], "--out-result-summary") == 0 && i + 1 < argc) {
